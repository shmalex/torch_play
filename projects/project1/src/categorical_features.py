...
class CategoricalFeatures:
	def __init__(self,
		...
		encoding_type: label, binry oneHot
		"""
		self.df = df
	...
	def _label_encodings(self):
		# for every column
		for c in tqdm.tqdm(self.cat_feat):
	...
	def transform(self, df:pd.DataFrame):
		...
		if self.enc_type == 'label':
			for c, lbl in tqdm.tqdm(self.label_encoders.items()):
				df.loc[:,c] = lbl.transform(df[c].values)
			return df
		elif self.enc_type == 'binary':
			output = df.copy(deep=True)
			for c, lbl in tqdm.tqdm(self.binary_encoder.items()):
